...
namespace Id3Lib
{
    ...
    internal class TextBuilder
    {
        ...
        public static string ReadUTF16( byte[] frame, ref int index )
        {
            ...
            else if( frame[index] == 0xfe && frame[index + 1] == 0xff ) // Big Endian
            {
                index += 2;
                return ReadUTF16BE(frame, ref index);
            }
            else if( frame[index] == 0xff && frame[index + 1] == 0xfe ) // Little Endian
            {
                index += 2;
                return ReadUTF16LE(frame, ref index);
            }
            else if( frame[index] == 0x00 && frame[index + 1] == 0x00 ) // empty string
            {
                index += 2;
                return "";
            }
